...
def home_view(request):
    ...
    todas = (
        Disciplina.objects.prefetch_related("turma_disciplina__professor")
        .all()
        .order_by("codigo")
    )
...
def disciplina_detalhe(request, pk):
    disciplina = get_object_or_404(
        Disciplina.objects.prefetch_related("grupos_requisitos__disciplinas"), pk=pk
    )
    ...
    alunos_count = (
        Student.objects.filter(grades__disciplina=disciplina).distinct().count()
    )
    return render(
        request,
        "core/disciplina_detalhe.html",
        {
...
def professores(request):
    ...
    paginator = Paginator(
        todos, 3
    )  # o certo é 15, mas como n tem professor suficiente coloquei menos só para testar
    page_number = request.GET.get("page")
    ...
    return render(
        ...
        {"professores": professores_paginados, "search_query": search_query},
    )
...
def reportar_avaliacoes(request, avaliacao_id):
    ...
    if request.method == "POST":
        ...
        if motivo == "outros" and not descricao:
            messages.error(
                request, "A descrição é obrigatória quando o motivo for 'Outros'."
            )
            return render(
                request, "core/reportar-avaliacoes.html", {"avaliacao": avaliacao}
            )

        Denuncia.objects.create(avaliacao=avaliacao, motivo=motivo, descricao=descricao)
        messages.success(
            request, "Avaliação reportada com sucesso! Analisaremos o caso."
        )
...
def grade_view(request):
    ...
    turmas = (
        Turma.objects.filter(disciplina_id__in=disc_ids)
    ...
    disc_sem_turma = Disciplina.objects.filter(id__in=disc_ids).exclude(
        id__in=disc_com_turma_ids
    )
...
def matricula_view(request):
    ...
    student = (
        get_object_or_404(Student, user=request.user)
        if request.user.is_authenticated
        else Student.objects.first()
    )
    matriculas = Matricula.objects.filter(aluno=student)
    matriculadas_ids = list(matriculas.values_list("disciplina_id", flat=True))
    total_creditos = (
        Disciplina.objects.filter(id__in=matriculadas_ids).aggregate(
            total=Sum("creditos")
        )["total"]
        or 0
    )
    turmas_matriculadas = (
        Turma.objects.filter(disciplina_id__in=matriculadas_ids)
        .select_related("disciplina", "professor")
        .order_by("horario")
    )
    ...
    disciplinas_com_turmas = [
        (
            ...
            list(
                {
                    t.id: t
                    for t in Turma.objects.filter(disciplina=d)
...
def inscrever_disciplina(request, disciplina_id):
    ...
    if request.method == "POST":
        Matricula.objects.get_or_create(
            aluno=student, disciplina=disciplina, defaults={"semestre": 1, "ano": 2026}
        )
...
def minhas_avaliacoes_prof(request, id_professor):
    ...
    return render(
        request,
        "core/minhas-avaliacoes-prof.html",
        {
...
def solicitar_disciplina(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            sol.save()
            messages.success(
                request, "Solicitação enviada! Aguarde a análise do administrador."
            )
...
def painel_admin(request):
    ...
    if disc_busca:
        disc_qs = disc_qs.filter(
            Q(nome__icontains=disc_busca) | Q(codigo__icontains=disc_busca)
        )
    if disc_status:
...
def admin_curriculo_detalhe(request, cur_id):
    ...
    if f_busca:
        items = items.filter(
            Q(disciplina__nome__icontains=f_busca)
            | Q(disciplina__codigo__icontains=f_busca)
        )
    if f_creditos:
...
def admin_curriculo_update_item(request, item_id):
    ...
    if request.method == "POST":
        ...
        item.periodo_recomendado = int(
            request.POST.get("periodo_recomendado", item.periodo_recomendado)
        )
        item.save()
...
def progressao_academica(request):
    ...
    items = curriculo.items.select_related("disciplina").order_by(
        "periodo_recomendado", "tipo", "disciplina__nome"
    )

    matriculas_map = {
        m.disciplina_id: m for m in Matricula.objects.filter(aluno=student)
    }
    concluidas_ids = {
        did
        for did, m in matriculas_map.items()
        if m.status == Matricula.StatusMatricula.CONCLUIDO
    }
    ...
    for item in items:
        ...
        if matricula:
            ...
            prereqs = item.disciplina.grupos_requisitos.prefetch_related(
                "disciplinas"
            ).filter(tipo="PRE")
            bloqueado = False
            for grupo in prereqs:
                ids = list(grupo.disciplinas.values_list("id", flat=True))
                if grupo.operador == "AND" and not all(
                    i in concluidas_ids for i in ids
                ):
                    bloqueado = True
                    break
    ...
    obrig_pct = (
        round(stats["obrig_ok"] / stats["obrig_total"] * 100)
        if stats["obrig_total"]
        else 0
    )
    optat_pct = (
        round(stats["optat_ok"] / stats["optat_total"] * 100)
        if stats["optat_total"]
        else 0
    )

    return render(
        request,
        "core/progressao.html",
        {
            "student": student,
            "curriculo": curriculo,
...
def admin_dashboard(request):
    """Painel principal do administrador - dashboard com estatísticas."""
    from django.contrib.auth.models import User

    total_matriculas = Matricula.objects.count()
    total_estudantes = Student.objects.count()

    # Avaliação média de todas as avaliações
    avaliacao_media = Avaliacao.objects.aggregate(media=Avg("nota_disc"))["media"]
    ...
    solicitacoes_recentes = SolicitacaoDisciplina.objects.select_related(
        "solicitante"
    ).order_by("-criado_em")[:5]

    # Avaliações pendentes de moderação (com denúncias)
    avaliacoes_pendentes_count = Denuncia.objects.count()

    # Disciplinas ativas
    disciplinas_ativas = Disciplina.objects.filter(status="aprovado").order_by("nome")[
        :10
    ]

    # Anotar cada disciplina com contagem de alunos e vagas
    disciplinas_list = []
    for disc in disciplinas_ativas:
        alunos_count = (
            Student.objects.filter(grades__disciplina=disc).distinct().count()
        )
        vagas_totais = 0
        turmas = disc.turma_disciplina.all()
        for turma in turmas:
            vagas_totais += 40  # Valor padrão de vagas por turma
        ...
        disciplinas_list.append(
            {
                ...
                "alunos_matriculados": alunos_count,
                "vagas_totais": vagas_totais if vagas_totais > 0 else alunos_count,
                "turma_disciplina": turmas,
            }
        )
    ...
    return render(
        ...
        "core/admin_dashboard.html",
        {
            ...
            "avaliacoes_pendentes_count": avaliacoes_pendentes_count,
            "disciplinas_ativas": disciplinas_list,
        },
    )
...
def admin_avaliacoes(request):
    """Gerenciamento de avaliações denunciadas."""
    status = request.GET.get("status", "pendente").strip()

    # Buscar denúncias com status filtrado
    denuncias = Denuncia.objects.select_related(
        "avaliacao__aluno__user", "avaliacao__disciplina"
    ).order_by("-data_denuncia")

    # Para simplificar, como Denuncia não tem campo status, consideramos todas como pendentes
    # e filtramos por data ou outras propriedades
    if status == "pendente":
        denuncias = denuncias.all()
    elif status == "aprovado":
        denuncias = denuncias.none()  # Ajustar quando implementar status
    elif status == "rejeitado":
        denuncias = denuncias.none()  # Ajustar quando implementar status

    return render(
        request,
        "core/admin_avaliacoes.html",
        {
            "denuncias": denuncias,
...
@user_passes_test(_is_superuser, login_url="/login/")
def admin_avaliacao_aprovar(request, denuncia_id):
    """Aprovar avaliação denunciada."""
    denuncia = get_object_or_404(Denuncia, id=denuncia_id)
    if request.method == "POST":
        messages.success(request, "Avaliação aprovada e mantida publicada.")
    return redirect("admin_avaliacoes")


@user_passes_test(_is_superuser, login_url="/login/")
def admin_avaliacao_rejeitar(request, denuncia_id):
    """Rejeitar avaliação denunciada."""
    denuncia = get_object_or_404(Denuncia, id=denuncia_id)
    if request.method == "POST":
        # Opcionalmente deletar a avaliação
...
def admin_disciplinas(request):
    ...
    disciplinas = (
        Disciplina.objects.prefetch_related("turma_disciplina__professor")
        .annotate(
            alunos_matriculados=Count("matricula_disciplina__aluno", distinct=True)
        )
        .order_by("nome")
    )
    ...
    if dept_query:
        disciplinas = disciplinas.filter(codigo__istartswith=dept_query)

    if status_query:
        disciplinas = disciplinas.filter(status=status_query)
    ...
    # Adicionar property vagas_totais dinamicamente
    for disc in disciplinas:
        disc.vagas_totais = max(40 * disc.turma_disciplina.count(), 40)

    paginator = Paginator(disciplinas, 15)
    page_number = request.GET.get("page")
    disciplinas_page = paginator.get_page(page_number)

    return render(
        request,
        "core/admin_disciplinas.html",
        {
...
@user_passes_test(_is_superuser, login_url="/login/")
def admin_validar_disciplinas(request):
    """Validar/aprovar disciplinas que estão sob avaliação."""
    search_query = request.GET.get("search", "").strip()
    dept_query = request.GET.get("departamento", "").strip()

    # Mostrar apenas disciplinas que estão sob avaliação
    disciplinas = (
        Disciplina.objects.prefetch_related("turma_disciplina__professor")
        .filter(status=Status.SOB_AVALIACAO)
        .annotate(
            alunos_matriculados=Count("matricula_disciplina__aluno", distinct=True)
        )
        .order_by("nome")
    ...
    # Obter departamentos únicos
    departamentos = set()
    for disc in Disciplina.objects.filter(status=Status.SOB_AVALIACAO):
        dept = disc.codigo[:3]
        departamentos.add(dept)
    departamentos = sorted(departamentos)
    ...
    return render(
        request,
        "core/admin_validar_disciplinas.html",
        {
            "disciplinas": disciplinas_page,
            "search_query": search_query,
            "dept_query": dept_query,
...
@user_passes_test(_is_superuser, login_url="/login/")
def admin_aprovar_disciplina(request, disciplina_id):
    """Aprovar/validar uma disciplina."""
    disciplina = get_object_or_404(Disciplina, id=disciplina_id)
    if request.method == "POST":
        disciplina.status = Status.APROVADO
        disciplina.save()
        messages.success(
            request, f'Disciplina "{disciplina.nome}" aprovada com sucesso!'
        )
    return redirect("admin_validar_disciplinas")


@user_passes_test(_is_superuser, login_url="/login/")
def admin_rejeitar_disciplina(request, disciplina_id):
    """Rejeitar uma disciplina (cancelar)."""
    disciplina = get_object_or_404(Disciplina, id=disciplina_id)
    if request.method == "POST":
        disciplina.status = Status.CANCELADO
        disciplina.save()
        messages.success(request, f'Disciplina "{disciplina.nome}" rejeitada.')
    return redirect("admin_validar_disciplinas")


@user_passes_test(_is_superuser, login_url="/login/")
def admin_validar_professores(request):
    """Validar/aprovar professores que estão pendentes."""
    search_query = request.GET.get("search", "").strip()
    dept_query = request.GET.get("departamento", "").strip()

    # Mostrar apenas professores que estão pendentes
    professores = Professor.objects.filter(
        status=Professor.StatusProfessor.PENDENTE
    ).order_by("nome")

    if search_query:
        professores = professores.filter(
            Q(nome__icontains=search_query) | Q(departamento__icontains=search_query)
        )

    if dept_query:
        professores = professores.filter(departamento__icontains=dept_query)

    # Obter departamentos únicos
    departamentos = set()
    for prof in Professor.objects.filter(status=Professor.StatusProfessor.PENDENTE):
        departamentos.add(prof.departamento)
    departamentos = sorted(departamentos)

    paginator = Paginator(professores, 15)
    page_number = request.GET.get("page")
    professores_page = paginator.get_page(page_number)

    return render(
        request,
        "core/admin_validar_professores.html",
        {
...
@user_passes_test(_is_superuser, login_url="/login/")
def admin_aprovar_professor(request, professor_id):
    """Aprovar um professor."""
    professor = get_object_or_404(Professor, id=professor_id)
    if request.method == "POST":
        professor.status = Professor.StatusProfessor.APROVADO
        professor.save()
        messages.success(request, f'Professor "{professor.nome}" aprovado com sucesso!')
    return redirect("admin_validar_professores")


@user_passes_test(_is_superuser, login_url="/login/")
def admin_rejeitar_professor(request, professor_id):
    """Rejeitar um professor."""
    professor = get_object_or_404(Professor, id=professor_id)
    if request.method == "POST":
        professor.status = Professor.StatusProfessor.REJEITADO
        professor.save()
        messages.success(request, f'Professor "{professor.nome}" rejeitado.')
    return redirect("admin_validar_professores")


@user_passes_test(_is_superuser, login_url="/login/")
def admin_denuncias_disciplinas(request):
    """Ver e gerenciar denúncias de disciplinas."""
    status = request.GET.get("status", "pendente").strip()

    denuncias = DenunciaDisciplina.objects.select_related(
        "disciplina", "denunciante"
    ).order_by("-data_denuncia")

    if status == "pendente":
        denuncias = denuncias.filter(status=DenunciaDisciplina.StatusDenuncia.PENDENTE)
    elif status == "validado":
        denuncias = denuncias.filter(status=DenunciaDisciplina.StatusDenuncia.VALIDADO)
    elif status == "rejeitado":
        denuncias = denuncias.filter(status=DenunciaDisciplina.StatusDenuncia.REJEITADO)

    paginator = Paginator(denuncias, 10)
    page_number = request.GET.get("page")
    denuncias_page = paginator.get_page(page_number)

    return render(
        request,
        "core/admin_denuncias_disciplinas.html",
        {
            "denuncias": denuncias_page,
            "current_status": status,
        },
    )
...
@user_passes_test(_is_superuser, login_url="/login/")
def admin_validar_denuncia_disciplina(request, denuncia_id):
    """Validar denúncia de disciplina (manter disciplina aprovada)."""
    denuncia = get_object_or_404(DenunciaDisciplina, id=denuncia_id)
    if request.method == "POST":
        denuncia.status = DenunciaDisciplina.StatusDenuncia.VALIDADO
        denuncia.save()
        messages.success(
            request, "Denúncia validada. Disciplina mantida como aprovada."
        )
    return redirect("admin_denuncias_disciplinas")


@user_passes_test(_is_superuser, login_url="/login/")
def admin_rejeitar_denuncia_disciplina(request, denuncia_id):
    """Rejeitar denúncia de disciplina (cancelar disciplina)."""
    denuncia = get_object_or_404(DenunciaDisciplina, id=denuncia_id)
    if request.method == "POST":
        denuncia.status = DenunciaDisciplina.StatusDenuncia.REJEITADO
        denuncia.save()
        # Rejeitar a disciplina também
        denuncia.disciplina.status = Status.CANCELADO
        denuncia.disciplina.save()
        messages.success(request, "Denúncia aceita. Disciplina foi cancelada.")
    return redirect("admin_denuncias_disciplinas")


@user_passes_test(_is_superuser, login_url="/login/")
def admin_aprovar_disciplina_geral(request, disciplina_id):
    """Aprovar uma disciplina da página de gerenciamento geral."""
    disciplina = get_object_or_404(Disciplina, id=disciplina_id)
    if request.method == "POST":
        disciplina.status = Status.APROVADO
        disciplina.save()
...
@user_passes_test(_is_superuser, login_url="/login/")
def admin_remover_disciplina_geral(request, disciplina_id):
    """Remover (cancelar) uma disciplina da página de gerenciamento geral."""
    disciplina = get_object_or_404(Disciplina, id=disciplina_id)
    if request.method == "POST":
        disciplina.status = Status.CANCELADO
        disciplina.save()
